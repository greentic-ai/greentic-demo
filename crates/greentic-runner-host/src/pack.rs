...
fn map_tenant_ctx(ctx: TenantCtx, default_env: &str) -> TypesTenantCtx {
    ...
    let env_id = EnvId::try_from(env.as_str()).unwrap_or_else(|err| {
        tracing::warn!(error = %err, value = %env, "invalid env id, falling back to 'local'");
        EnvId::try_from("local").expect("static fallback env id")
    });

    let tenant_id = TenantId::try_from(ctx.tenant.as_str()).unwrap_or_else(|err| {
        tracing::warn!(error = %err, tenant = %ctx.tenant, "invalid tenant id, using fallback");
        TenantId::try_from("default").expect("static fallback tenant id")
    });

    let mut mapped = TypesTenantCtx::new(env_id, tenant_id);

    if let Some(team) = ctx.team {
        match TeamId::try_from(team.as_str()) {
            Ok(team_id) => {
                mapped.team = Some(team_id.clone());
                mapped.team_id = Some(team_id);
            }
            Err(err) => {
                tracing::warn!(error = %err, value = %team, "invalid team id skipped");
            }
        }
    }

    if let Some(user) = ctx.user {
        match UserId::try_from(user.as_str()) {
            Ok(user_id) => {
                mapped.user = Some(user_id.clone());
                mapped.user_id = Some(user_id);
            }
            Err(err) => {
                tracing::warn!(error = %err, value = %user, "invalid user id skipped");
            }
        }
    }

    mapped.trace_id = ctx.trace_id;
    mapped
}
