...
fn types_tenant_ctx(ctx: &FlowContext<'_>, default_env: &str) -> TypesTenantCtx {
    let env_id = EnvId::try_from(default_env).unwrap_or_else(|err| {
        tracing::warn!(error = %err, value = default_env, "invalid env id, falling back to 'local'");
        EnvId::try_from("local").expect("static fallback env id")
    });
    let tenant_id = TenantId::try_from(ctx.tenant).unwrap_or_else(|err| {
        tracing::warn!(error = %err, tenant = ctx.tenant, "invalid tenant id, using sanitized fallback");
        TenantId::try_from("default").expect("static fallback tenant id")
    });

    let mut output = TypesTenantCtx::new(env_id, tenant_id);
    output.flow_id = Some(ctx.flow_id.to_string());
    output.node_id = ctx.node_id.map(|node| node.to_string());
